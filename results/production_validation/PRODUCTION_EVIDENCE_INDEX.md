...
| PE-02 | `cohort_recomputed_20260902/matched_window_cohort_recomputed.json` and `matched_window_pairs.csv` | reproducible one-to-one screening of 512 load-matched two-hour window pairs | independent-window inference or isolated causal effect |
...
- The PE-02 summary records SHA-256 values for all 24 monthly pre-retrofit
  query files and the post-retrofit query file. The public pair table contains
  derived window metrics and source timestamps, not proprietary raw traces.
