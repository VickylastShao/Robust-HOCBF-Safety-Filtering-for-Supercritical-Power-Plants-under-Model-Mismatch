"""Test rollout with different k_gains to find configuration that maintains QP feasibility.

Key insight: k_p=[0.5,0.5] gives b_min=0.186 at equilibrium, which is too tight.
Increasing k_gains gives much more QP headroom.

This script tests k_p=[2.0,2.0] with MC=True + scenario-specific GP,
running a full training+evaluation loop on S1:Heat.
"""
import sys
...
# Test configurations
configs_to_test = [
    # (label, k_p, k_h, mc, scenario_specific, eps_floor)
    ("k2_mc_true_spec", [2.0, 2.0], [3.0], True, True, 0.0),
    ("k1_mc_true_spec", [1.0, 1.0], [3.0], True, True, 0.0),
    ("k2_mc_false_mixed", [2.0, 2.0], [3.0], False, False, 0.0),
    ("k1_mc_false_mixed", [1.0, 1.0], [3.0], False, False, 0.0),
]

results = {}

for label, k_p, k_h, mc, spec_gp, eps_floor in configs_to_test:
    print(f"\n{'='*60}")
    print(f"Testing: {label}")
    print(f"  k_p={k_p}, k_h={k_h}, MC={mc}, spec_gp={spec_gp}, eps_floor={eps_floor}")
    print(f"{'='*60}")
    ...
    safety_layer = _make_robust_hocbf(dynamics, constraint, gp, u0,
                                       epsilon_kappa=1.0,
                                       k_pressure=tuple(k_p),
                                       k_enthalpy=tuple(k_h),
                                       u_max=100.0,
    ...
    for ep in range(n_episodes):
        ...
        # GP update for rocbf_net (always update GP for these tests)
    ...
    result = {
        'label': label,
        'config': {'k_p': k_p, 'k_h': k_h, 'use_mean_correction': mc,
...
with open(os.path.join(output_dir, 's1_heat.json'), 'w') as f:
...
for label, r in results.items():
    ...
    print(f"  {label}: viol={r['violation_rate'][0]:.4f}, "
