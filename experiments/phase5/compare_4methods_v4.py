"""Comprehensive comparison: PPO vs HOCBF vs GP-HOCBF vs RHOCBF under all scenarios.
100 steps, 1 seed, 4 methods.
"""
import sys, warnings
...
def main():
    print("4-Method Comparison — Redesigned Scenarios v4 (100 steps)")
    print("=" * 80)
    ...
    header = f"{'Scenario':<16} {'PPO':>8} {'HOCBF':>8} {'GP-HOCBF':>10} {'RHOCBF':>8}"
    print(header)
    print("-" * len(header))
    ...
    for si, scenario in enumerate(SCENARIOS):
        ...
        # PPO (no safety layer)
        ppo_viol, _ = eval_method(model, d, c, x0, u0, None)

        # HOCBF (nominal)
        hocbf = _make_hocbf_5th(d, c, u0)
        hocbf_viol, hocbf_qp = eval_method(model, d, c, x0, u0, hocbf)

        # GP-HOCBF (mean correction only)
        gp_hocbf = _make_robust_hocbf_5th(d, c, gp, u0,
                                           epsilon_kappa=0.0, use_mean_correction=True)
        gp_hocbf_viol, gp_hocbf_qp = eval_method(model, d, c, x0, u0, gp_hocbf)

        # RHOCBF (mean correction + epsilon)
        rhocbf = _make_robust_hocbf_5th(d, c, gp, u0,
                                         epsilon_kappa=1.0, use_mean_correction=True)
        rhocbf_viol, rhocbf_qp = eval_method(model, d, c, x0, u0, rhocbf)

        print(f"{label:<16} {ppo_viol:>7.1f}% {hocbf_viol:>7.1f}% {gp_hocbf_viol:>9.1f}% {rhocbf_viol:>7.1f}%")
        print(f"{'':16} {'':8} QP={hocbf_qp:>5.1f}%  QP={gp_hocbf_qp:>5.1f}%  QP={rhocbf_qp:>5.1f}%")
