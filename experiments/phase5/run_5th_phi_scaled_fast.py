"""Full 5th-order CCS safety comparison — Φ-scaled nonlinear rollout (optimized).

Optimized version of run_5th_phi_scaled.py:
- Reduced training episodes: 30→10 (PPO policy is not the focus; safety filter dominates)
- Reduced eval steps: 500→200 (sufficient for convergence detection)
- GP pretrain: 3000→2000 (still adequate for 5D space)
- Uses step_stabilized_phi_scaled for rollout
- Uses g_phi_scaled in CBF construction (use_phi_scaled_g=True)
- Uses epsilon_kappa=0.5 for PPO-RHOCBF and RoCBF-Net
- S6:Fuel perturbation revised: Δf_τ=0

Methods (6): PPO, PPO-HOCBF, PPO-GP-HOCBF, PPO-RHOCBF, RoCBF-Net, LQR+RHOCBF
...
Usage:
    ...
    python experiments/phase5/run_5th_phi_scaled_fast.py
    python experiments/phase5/run_5th_phi_scaled_fast.py --methods ppo ppo_hocbf ppo_rhocbf
    python experiments/phase5/run_5th_phi_scaled_fast.py --conditions S1 S3
"""
...
LOAD_RATIO = 1.0
N_TRAIN = 10       # Reduced from 30 (safety filter dominates; PPO just explores)
N_GP_PRETRAIN = 2000  # Reduced from 3000
N_EVAL = 200        # Reduced from 500 (sufficient for violation rate estimation)
N_SEEDS = 5
...
def evaluate(method_name, scenario, seed, n_train=N_TRAIN, n_eval=N_EVAL):
    ...
    if method_name == 'ppo_hocbf':
        ...
        safety_layer = _make_robust_hocbf_5th(
            dynamics, constraint, gp, u0,
            epsilon_kappa=0.5, use_mean_correction=True, use_phi_scaled_g=True)
        gp_type = 'scenario_specific'
    ...
    if method_name not in ('lqr_rhocbf',):
...
if __name__ == '__main__':
    ...
    if args.methods:
        ...
        methods = ['ppo', 'ppo_hocbf', 'ppo_gp_hocbf', 'ppo_rhocbf', 'rocbf_net', 'lqr_rhocbf']
