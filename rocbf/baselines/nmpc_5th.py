"""Nonlinear MPC baseline for 5th-order CCS control with Φ-scaled dynamics.
...
NumPy (no JAX calls in the solve loop).
...
Φ-scaling effects, and perturbation scenarios).

Output is linearized around equilibrium:
    y_k ≈ y0 + C_eff @ (x_k - x0) + D_eff @ v_k

Key differences from 3rd-order NMPC:
- 5 states (r_B, p_m, h_m, N_e, τ_f) instead of 3
- 6 CBF constraints (including power high/low) instead of 4
- Φ-scaled B_d in prediction for better accuracy under state-dependent
  control effectiveness variation
"""
import time
import numpy as np
import jax
...
class NMPCController5th:
    """NMPC for 5th-order CCS control with state disturbance correction.

    Parameters
    ----------
    dynamics : USCCSDynamics5th
        5th-order CCS dynamics model.
    constraint : CCSConstraints5th
        Safety constraints (6: pressure high/low, enthalpy high/low, power high/low).
    horizon : int
        Prediction horizon N (steps). Default 10.
    ...
    def __init__(self, dynamics, constraint, horizon: int = 10,
        ...
        # Constraint gradients at equilibrium (6 constraints)
        self.dh = {}
        self.h0 = {}
        constraint_defs = [
        ...
        for name, h_fn in constraint_defs:
            self.dh[name] = np.array(jax.grad(h_fn)(x0_j))
            self.h0[name] = float(h_fn(x0_j))

        # Φ(p_m) at equilibrium for B_d scaling
        self._phi0 = float(dynamics.fluid_property(dynamics._x0[1]))

        # State disturbance estimate (additive on dx, 5D)
    ...
    def update_disturbance(self, x_actual):
        ...
        Compares the actual next state with the nominal linearized prediction
        (using fixed B_d). The difference captures all mismatch sources:
        Φ-scaling, uncertainty scenarios, etc.
        """
        if self._prev_x is None:
            return

        dx_prev = np.array(self._prev_x[:5]) - self.x0
        v_prev = np.array(self._prev_v)

        # Predict using nominal linear model (no Φ-scaling)
        dx_pred_no_d = self.A_d @ dx_prev + self.B_d @ v_prev
        dx_actual = np.array(x_actual[:5]) - self.x0
    ...
    def compute_action(self, x, y_ref=None):
        ...
        -------
        v_opt : jnp.ndarray
            Deviation control for step_stabilized_phi_scaled (3,).
        """
        x_np = np.array(x[:5])
        ...
        # Use nominal B_d — disturbance correction handles Φ-scaling mismatch
        # (matching original 3rd-order NMPC design)
        ...
        # Linearized CBF constraints at each prediction step
        constraints = []
