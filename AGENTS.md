...
```bash
# Preferred: run on the LAN GPU servers through the helper.
python3 /home/shao/.agents/skills/lan-gpu-resources/scripts/lan_gpu.py status
python3 /home/shao/.agents/skills/lan-gpu-resources/scripts/lan_gpu.py --host gpu205 run RoCBF-Net -- python -m pytest tests/ -q

# Remote project layout on both GPU servers:
...
- LAN GPU servers, each with one RTX 4090 24GB (`CudaDevice id=0`):
  - `gpu205` = `192.168.102.205`, remote project `/home/gpu/sz_workspace/RoCBF-Net`
  - `gpu206` = `192.168.102.206`, remote project `/home/gpu/sz_workspace/RoCBF-Net`
- Use the project-local remote virtual environment: `/home/gpu/sz_workspace/RoCBF-Net/.venv`
- Prefer `gpu205` for default runs. `gpu206` is usable but has shown intermittent SSH reset under concurrent connections; use serial helper commands there.
- Python ≥3.11
