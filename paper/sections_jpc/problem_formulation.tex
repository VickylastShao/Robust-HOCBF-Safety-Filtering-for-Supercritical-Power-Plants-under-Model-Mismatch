...
We model each component of $\Deltaf_j: \R^n \to \R$ as an independent GP with Mat\'{e}rn-5/2 kernel. Given a dataset $\calD = \{(x_i, y_i)\}_{i=1}^N$, the GP posterior yields mean $\mu_{\mathrm{GP},j}(x)$ and variance $\sigma_{\mathrm{GP},j}^2(x)$. The GP-UCB confidence bound~\cite{srinivas2010gp} guarantees:
\begin{equation}
...
The GP posterior variance provides a calibrated uncertainty estimate: the GP-UCB confidence bound~\eqref{eq:gp_bound} holds with the chosen $\beta$. Scenario-specific training is used to support this calibration at deployment; calibration diagnostics are discussed in Section~\ref{sec:deployment}.
\end{assumption}
