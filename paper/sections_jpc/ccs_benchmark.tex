...
\begin{enumerate}
    ...
    \item \textbf{S4: Nonlinear fouling}. Quadratic pressure dependence.
...
Eight methods are compared: (1)~PPO (no safety), (2)~PPO-Lagrangian, (3)~NMPC with offset-free disturbance estimation (horizon $N=5$, SLSQP solver), (4)~PPO-CBF (first-order CBF, incorrect $m$), (5)~PPO-HOCBF (correct $m$, no GP), (6)~PPO-GP-HOCBF (GP mean correction, $\epsilon=0$), (7)~PPO-RHOCBF (fixed scenario-specific GP, $\epsilonKappa=1.0$), (8)~LQR+RHOCBF (same filter, classical upstream controller). Training: 200 PPO episodes $\times$ 200 steps (40,000 total environment steps). PPO provides a learned upstream controller; the safety guarantee is controller-agnostic---LQR+RHOCBF achieves identical safety. Evaluation: 5 seeds $\times$ 50 episodes $\times$ 500 steps (125,000 total steps) under $\Phi$-scaled nonlinear rollout. GP pretraining: $N=2000$ points per scenario. Primary safety metric: episode-level CBF violation rate with 95\% Wilson CI. Process control performance metrics: tracking error (IAE), constraint violation magnitude, QP intervention rate, and per-step solve latency.
