...
\begin{align}
    ...
    \delta_i(x) &= L_{\Delta \hat{f}} \hat{\psi}_{i-1}^0 + (L_{\fhat} + k_i) \delta_{i-1} + \underbrace{L_{\Delta \hat{f}} \delta_{i-1}}_{\text{cross term}}, \quad i \geq 2 \label{eq:delta_recursive}
\end{align}
...
\begin{align}
    \sigma_1(x) &= \beta \sum_{j=1}^{n} \left|\frac{\partial h}{\partial x_j}\right| \sigmaGPj{j}(x) \label{eq:sigma1} \\
    \sigma_i(x) &= \beta \sum_{j=1}^{n} \left|\frac{\partial \hat{\psi}_{i-1}^0}{\partial x_j}\right| \sigmaGPj{j}(x) + (|L_{\fhat}|_{\mathrm{op}} + k_i) \sigma_{i-1}(x) + \sigma_{\mathrm{cross}}^{(i)}(x), \; i \geq 2 \label{eq:sigma_recursive} \\
