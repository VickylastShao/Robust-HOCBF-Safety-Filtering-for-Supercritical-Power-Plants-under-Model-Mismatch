...
| The tune rule selects $\epsilon_\kappa=0.02$ for S3 before confirmation; the composite primary row combines this S3 value with the mean-only endpoint ($\epsilon_\kappa=0$) in the other six conditions and records 0/17,500 observed violations. | Sections 4.2--4.4; Supplemental Material | `results/phase5_commissioning_kappa_tune_20260831/selection_summary.json`; `results/phase5_primary_kappa002_20260831/` | Stored evidence; only S3 has the reported tune/test factor selection |
...
| Full implemented-margin operation can lose feasibility under actuator limits. | Section 4.2; Supplemental Material | Primary S3 endpoint records in `results/phase5_qpax_x64_primary_a_20260831/` | Stored mechanism diagnostic |
