...
\textbf{Step 4 (Forward invariance).} For $v^*$ satisfying $A(x)v^* \leq b(x)-\sigmatotal(x)$, the definition $\hat{\psi}_m^0(x,v)=b(x)-A(x)v$ gives $\hat{\psi}_m^0(x,v^*)\geq\sigmatotal(x)$. Hence $\psi_m^{\mathrm{true}}(x,v^*) = \hat{\psi}_m^0(x,v^*) + \Delta_{\psi}(x,v^*) \geq \sigmatotal(x) - |\Delta_{\psi}(x,v^*)| \geq 0$. Because $x(0)\in\calC$, all lower-order recursive conditions in the main article's definition of $\calC$ are satisfied initially. The HOCBF forward-invariance theorem cited there therefore renders $\calC$ forward invariant.
\end{proof}
...
\begin{equation}
    \sigma_{\mathrm{ctrl}}(x) = \beta v_{\max} \sum_{j} \left\|\frac{\partial (L_{g_v} L_{\fhat}^{m-1} h)}{\partial x_j}\right\|_2 \sigmaGPj{j}(x)
    \label{eq:sigma_ctrl_app}
\end{equation}
where $g_v=g_0S_u$ and $v_{\max}=\sup_{v\in[-1,1]^3}\|v\|_2=\sqrt{3}$. This term is evaluated separately for each barrier row and included in that row's total margin; Theorem~1 does not assume a fixed ratio between $\sigma_{\mathrm{ctrl}}$ and $\sigmatotal$.
