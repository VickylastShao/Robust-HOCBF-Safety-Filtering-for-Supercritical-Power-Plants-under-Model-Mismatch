\subsection{S6. Triple-Integrator Validation ($m=3$)}
...
\begin{table}[htbp]
    \centering
    \caption{Triple Integrator $\epsilon$ Ablation under Nonlinear Perturbation ($m = 3$, 5 seeds $\times$ 50 episodes $\times$ 500 steps). Mean $\epsilon$ is the total per-step robustness margin; $\epsilon(x)$ statistics ($\mu=0.93$, $\max=3.39$, $\mathrm{std}=0.65$) are computed over sampled states. Results: mean $\pm$ std over 5 seeds.}
