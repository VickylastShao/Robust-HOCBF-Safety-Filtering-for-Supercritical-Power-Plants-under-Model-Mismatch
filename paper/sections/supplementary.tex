...
The selector fixes $\epsilon_\kappa=0$ before evaluating seeds 3--4. Each held-out seed contains 25{,}000 samples. Both seeds record zero violating samples and zero rejected QPs, giving pooled counts of 0/50{,}000 for both diagnostics. The descriptive 95\% Wilson upper reference for 0/15{,}000 tune-set violations is 0.0256\%; serial correlation prevents interpreting this value as an independent-trial confidence bound. No holdout result is used to change $\epsilon_\kappa$, and zero observed events are not presented as zero event probability.
...
\begin{table}[htbp]
    ...
    \small
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.34\textwidth}>{\centering\arraybackslash}p{0.18\textwidth}>{\centering\arraybackslash}p{0.18\textwidth}>{\centering\arraybackslash}p{0.20\textwidth}}
        \toprule
...
Across the seven confirmatory conditions, pooled research-pipeline time per controller update ranges from 3.19 to 6.71\,ms for the tune-selected RoCBF-SF row and from 3.75 to 20.82\,ms for NMPC. NMPC records no solver failure and 0/175{,}000 observed state-limit violations in this implementation. These benchmark wall-clock measurements are distinct from the deployed-CPU evidence: the confirmed controller exports report a maximum QP time of 6.234\,ms and a maximum total task time of 28.665\,ms against the 1000\,ms task deadline.
