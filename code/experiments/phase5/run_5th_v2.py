"""Full 5th-order CCS safety comparison — correct configuration.

5 methods × 7 conditions × 3 seeds
- PPO-GP-HOCBF: scenario-specific GP, ε=0 (mean correction only)
- PPO-RHOCBF: scenario-specific GP, ε=1.0 (mean correction + ε)
- RoCBF-Net: scenario-specific GP, ε=1.0 (same as PPO-RHOCBF for fixed GP)

Usage:
    conda activate jax_gpu
    cd /home/gpu/sz_workspace/RoCBF-Net
    python experiments/phase5/run_5th_v2.py
    python experiments/phase5/run_5th_v2.py --methods ppo ppo_hocbf ppo_gp_hocbf ppo_rhocbf
    python experiments/phase5/run_5th_v2.py --conditions S1 S2
"""
...
def evaluate(method_name, scenario, seed, n_train=N_TRAIN, n_eval=N_EVAL):
    """Evaluate method on 5th-order CCS with scenario-specific GP for all GP methods."""
    ...
    if method_name == 'ppo_hocbf':
        ...
        # Scenario-specific GP, mean correction + epsilon
        if scenario is not None:
            gp = _pretrain_gp_5th(LOAD_RATIO, n_pretrain=N_GP_PRETRAIN,
                                   key=jax.random.key(seed * 100 + 42),
                                   scenario=scenario, scenario_specific=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='safety_v2.json')
