"""Quick test: moderate GP + per-constraint differentiation on CCS.

Goal: find a GP calibration level where per-constraint differentiation
shows clear advantage WITHOUT QP infeasibility.
"""
    ...
    for ep in range(n_episodes):
        ...
        for t in range(n_steps):
            ...
            if t % 50 == 0:
                ...
                ep_eps.append([float(eps[i]) for i in range(len(eps))])
        ...
        if ep_eps:
            epsilon_values.append(np.mean(ep_eps, axis=0).tolist())
...
def main():
    ...
    # Goal: find GP where per-constraint ε differentiation matters
    # but ε is still small enough for QP feasibility
    gp_configs = []

    # 1. Scenario-specific with larger sigma_floor (increases ε uniformly)
    for sf in [1e-2, 5e-3, 1e-3]:
        gp_configs.append((f'scenario_sf{sf:.0e}', {
            'load_ratio': 1.0, 'delay_order': 0, 'n_pretrain': 2000,
            'sigma_floor': sf, 'scenario': 'heat_absorption',
            'scenario_specific': True,
        }))

    # 2. Moderate GP (partial coverage)
    gp_configs.append(('moderate', {
        'load_ratio': 1.0, 'delay_order': 0, 'n_pretrain': 500,
        'sigma_floor': 1e-4, 'scenario': 'heat_absorption',
        'scenario_specific': True, 'gp_coverage': 'moderate',
    }))

    # 3. Sparse GP
    gp_configs.append(('sparse', {
        'load_ratio': 1.0, 'delay_order': 0, 'n_pretrain': 200,
        'sigma_floor': 1e-4, 'scenario': 'heat_absorption',
        'scenario_specific': True, 'gp_coverage': 'sparse',
    }))

    qp_solver = DifferentiableQP(v_max=5.0)
    all_results = {}

    for gp_name, gp_cfg in gp_configs:
        print(f"\n{'='*70}", flush=True)
        print(f"GP: {gp_name}", flush=True)
        print(f"{'='*70}", flush=True)

        key, gk = jax.random.split(key)
        gp = _pretrain_gp(**gp_cfg, key=gk)
        ...
        # Test 3 key configurations: compositional, uniform_min, uniform_max
        test_configs = [
        ...
        for config_name, safety_layer in test_configs:
            t0 = time.time()
            result = evaluate_with_safety(
                model, eval_dyn, constraint, safety_layer, qp_solver,
                x0, u0, n_episodes=5, n_steps=200, seed=42)
            result['config_name'] = config_name
            result['gp_name'] = gp_name
            result['eps_p'] = eps_p
            result['eps_h'] = eps_h
            result['ratio'] = ratio
            all_results[f'{gp_name}_{config_name}'] = result
    ...
    print(f"{'GP':<20} {'Config':<16} {'CBF':>8} {'P_V':>8} {'H_V':>8} "
          f"{'Reward':>10} {'QP_I':>8} {'QP_F':>8} {'ε_p':>8} {'ε_h':>8} {'Ratio':>8}", flush=True)
    print("-" * 116, flush=True)

    for gp_name, _ in gp_configs:
        for config_name in ['compositional', 'constant_mean', 'uniform_min',
                            'uniform_max', 'no_epsilon']:
            k = f'{gp_name}_{config_name}'
            if k in all_results:
                r = all_results[k]
                print(f"{gp_name:<20} {config_name:<16} {r['cbf_violation_rate']:>8.4f} "
                      ...
                      f"{r['eps_p']:>8.4f} {r['eps_h']:>8.4f} {r['ratio']:>8.1f}", flush=True)
        print("", flush=True)

    # Save
    output_dir = 'results/phase5/per_constraint_diff/'
    os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, 'moderate_gp_results.json'), 'w') as f:
        def convert(obj):
