#!/usr/bin/env python
"""Fast LQR+RHOCBF comparison - uses fewer PPO epochs for speed.
Only runs S3 (seeds 3-4) and S4 to complete the comparison."""
import sys
...
def train_ppo(dynamics, constraint, x0, u0, seed=0):
    model = ActorCritic(n_obs=NX, n_act=3, hidden_dim=64, rngs=nnx.Rngs(seed))
    trainer = PPOTrainer(model, lr=1e-4, epochs=2, minibatch_size=64)
    key = jax.random.key(seed * 100)
    for ep in range(10):  # fewer episodes
...
# Complete S3 seeds 3-4 and S4 all seeds
scenarios_to_run = [
    ('coupled', 'S3:Coupled', [3, 4]),
    ('nonlinear', 'S4:Nonlinear', [0, 1, 2, 3, 4]),
]

results = {}
for scenario, label, seeds in scenarios_to_run:
    print(f'\n{"="*60}', flush=True)
    ...
    results[label] = {
        'seeds': seeds,
        'PPO': {'reward_mean': np.mean(ppo_rewards), 'reward_std': np.std(ppo_rewards),
                'cbf': np.mean(ppo_cbfs), 'qp': np.mean(ppo_qps)},
        'LQR': {'reward_mean': np.mean(lqr_rewards), 'reward_std': np.std(lqr_rewards),
