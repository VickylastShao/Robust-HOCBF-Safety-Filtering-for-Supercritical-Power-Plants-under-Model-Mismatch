"""PPO-RHOCBF baseline: k_p=[2.0,2.0], MC=True, scenario-specific GP, NO online GP update.

This is the control group for the k_gains ablation.
"""
import sys
...
# Configuration: k_p=[2.0,2.0], k_h=[3.0], MC=True, scenario-specific GP, NO GP update
k_p = (2.0, 2.0)
k_h = (3.0,)
mc = True
spec_gp = True

print("PPO-RHOCBF: k_p=[2.0,2.0], k_h=[3.0], MC=True, scenario-specific GP, NO online GP update")
...
for ep in range(n_episodes):
    ...
    # Log epsilon periodically (NO GP update for PPO-RHOCBF)
    if (ep + 1) % 50 == 0:
        eps_vals = [float(h.compute_epsilon(x0[:3])) for h in safety_layer.robust_hocbf_list]
        epsilon_log.append({
            'episode': ep + 1,
...
result = {
    'label': 'k2_mc_true_spec_rhocbf',
    'config': {'k_p': list(k_p), 'k_h': list(k_h), 'use_mean_correction': mc,
               'scenario_specific_gp': spec_gp, 'online_gp_update': False},
...
with open(os.path.join(output_dir, 'k2_rhocbf_s1_heat.json'), 'w') as f:
